import { BhmdLoggedOutMessageContainerComponent } from '../bhmd-logged-out-message-container/bhmd-logged-out-message-container';
import { BhmdCssAccordionComponent } from '../bhmd-css-accordion/bhmd-css-accordion.component';
...
@Component({
  selector: 'app-bhmd-offer-selector',
  standalone: true,
  imports: [BhmdLoggedOutMessageContainerComponent,BhmdCssAccordionComponent],
  templateUrl: './bhmd-offer-selector.html',
  styleUrl: './bhmd-offer-selector.css',
})
export class BhmdOfferSelectorComponent implements AfterViewInit {
  ...
  readonly flavors: Flavor[] = [
    {
      ...
      label: 'Chocolate',
      imgAlt: 'Chocolate',
      imgSrc: 'images/bhmd/BHMDStarterSet.webp',
    },
    {
      key: 'vanilla',
      label: 'Vanilla',
  ...
  lineItems = computed(() => {
    ...
    // --- template helper handlers (simple placeholders for now) ---
  selectPurchaseType(t: 'sub' | 'otp') {
    this.setPackageType(t);
  }

  openSubscribeDetails() {
    // placeholder: open modal, expand details, scroll, etc.
  }

  onReviewsClick(e: Event) {
    e.preventDefault();
    // optional: scroll to reviews section later
  }

  onKlarnaClick(e: Event) {
    e.preventDefault();
  }

  onGuaranteeLearnMore(e: Event) {
    e.preventDefault();
  }
